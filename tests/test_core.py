...
def test_tensor_multiplication(force_n):
    # F * d = Work = [M L^2 T^-2]
    distance_m = pt.q(2, "m")
...
def test_si_addition():
    # Basic validation of the 7-tuple SI dimensional representation
    length1 = pt.q(10, "m")
    length2 = pt.q(5, "m")
...
def test_maritime_units():
    # Validating nautical speed conversion
    speed = pt.q(1, "kn")

    # 1 knot ≈ 0.514444 m/s
    assert round(float(speed.data), 4) == 0.5144
