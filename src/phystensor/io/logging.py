...
def setup_logger(
    name: str = "phystensor", level: int = logging.INFO
) -> logging.Logger:
    """
    Initialize a logger that outputs structured JSON to stdout.
...
def log_dimension_error(
    error: Exception, dim_a: Any = None, dim_b: Any = None
) -> None:
    """
    ...
    logger.error(
        "Physics Validation Failed: " + str(error), extra=extra
)
