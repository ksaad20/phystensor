...
def sensor_array_processing():
    ...
    print(f"Sensor Grid Temps: {temperatures}")
    print(f"Data Shape: {temperatures.data.shape}")

    # 2. Vectorized Arithmetic
    # Apply a calibration offset to the entire array at once
    offset = pt.q(0.5, "K")
    calibrated_temps = temperatures + offset
    print(f"Calibrated Grid: {calibrated_temps}")
    ...
    print(f"Peak Energy: {max_energy}")
    ...
    print(f"Top Floor Readings: {top_floor_energy}")
